...
KafkaAscoltatore.prototype.publish = function publish(topic, message, done) {
  this._raiseIfClosed();
  var that = this;
  if(this._publisherStarting){
     debug("waiting for startup before publishing");
     setTimeout(function(){ that.publish(topic,message,done); }, 500);
     return;
  }
 ...
 if(this._producer !== undefined){
   ...
   return;
    ...
    this._producer.on('ready',send_message);
};
