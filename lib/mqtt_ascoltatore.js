  ...
  if (this._client === undefined) {
    that._opts.mqtt.createClient(that._opts.port, that._opts.host, function (err, client) {
      ...
      client.on("publish", function (packet) {
        ...
        var payload = null, index = false;

        payload = packet.payload;
        index = payload.indexOf("###json###");

        if(index === 0)  {
          debug("converting payload from JSON");
          payload = payload.replace("###json###", "");
          payload = JSON.parse(payload);
        }

        that._ascoltatore.publish(packet.topic, payload);
      });
...
MQTTAscoltatore.prototype.publish = function publish(topic, message, done) {
  ...
  if(typeof message !== "string") {
    debug("converting payload to JSON");
