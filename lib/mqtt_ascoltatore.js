...
MQTTAscoltatore.prototype._startConn = function () {
  var that = this, settings = null;
  if (this._client === undefined) {
    settings = { keepalive: that._opts.keepalive, clientId: util.buildIdentifier() };
...
MQTTAscoltatore.prototype.publish = function publish(topic, message, done) {
  ...
  this._client.publish(topic, String(message), { qos: 1 }, function () {
...
MQTTAscoltatore.prototype.close = function close(done) {
  ...
  if (!this._closed) {
    ...
    this._client.stream.once("close", function () {
      debug("closed");
      that.emit("closed");
