...
RedisAscoltatore.prototype.subscribe = function subscribe(topic, callback, done) {
  ...
  if(this._subs_counters[topic] === undefined) {
    this._subs_counters[topic] = 1;
  } else {
    this._subs_counters[topic] = this._subs_counters[topic] + 1;
  }
...
RedisAscoltatore.prototype.removeListener = function removeListener(topic, callback, done) {
  if(this._subs_counters[topic] === undefined) {
    // why the are we here?
    throw "RedisAscoltatore is in an unstable state"
  } else if(this._subs_counters[topic] === 1) {
    delete this._subs_counters[topic];
