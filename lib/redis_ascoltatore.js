...
function createConn(opts, connName) {
  ...
  if (conn === undefined) {
    ...
    var redisOpts = {};
    for (var key in opts)
      if (typeof opts[key] !== 'object' || key === 'redisOptions') redisOpts[key] = opts[key]; // Avoid circular reference TypeError
    if (redisOpts.redisOptions) {
      conn = opts.redis.createClient(redisOpts.redisOptions);
    } else {
