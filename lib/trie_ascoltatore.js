...
TrieAscoltatore.prototype.publish = function (topic, message, options, done) {
  ...
  var cbs = this._matcher.match(topic), i, cb, check = [];

  // do our own duplicate removal as we have to iterate the array anyway

  for (var i = 0; i < cbs.length; i++) {
    cb = cbs[i];
    if (cb._trie_ascoltatore_check !== check) {
      cb(topic, message, options);
      cb._trie_ascoltatore_check = check;
    }
  }
