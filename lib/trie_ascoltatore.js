...
TrieAscoltatore.prototype.registerDomain = function(domain) {
  ...
  this.publish = function(topic, message, options, done) {
    return domain.run(function() {
      return that._publish(topic, message, options, done);
    });
  };

  if (ascoltatori.used_ascoltatore === this) {
    ascoltatori.publish = this.publish;
  }
};
